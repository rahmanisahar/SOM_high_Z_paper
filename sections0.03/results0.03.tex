...
\label{sec: result}
    ...
    In this section we show the results of the neural networks trained using the K96 template spectra.
    Training with K96 templates results in networks that have regions corresponding to galaxies of known morphological type. 
   ...
   Based on the size of the data and SOM results, we found the optimum grid size to be $1\times22$ and $12\times12$ in 1D and 2D maps, respectively. 
    ...
    For each generated network, we compare the results with the K96 categorization.
 ...
 %           Increasing the size of maps means that even the smallest differences between galaxies have to be considered.
        ...
        \subsubsection{Classifying the galaxy sample}
         \label{sec: 1Dv}
            After training the networks as discussed in Sec.~\ref{sec: 1Dt}, we used them to classify the fitted SEDs of the sample of 142 galaxies from T12.
            ...
            \begin{figure}
                ...
                \begin{subfigure}[b]{0.5\textwidth}
                     ...
                     %\label{fig: 1by3Thits}
                \end{subfigure}
                \caption{Classification of fitted galaxy SEDs from T12 using the $1\times2$~networks trained from the K96 templates (Fig.~\ref{fig: 1by2T}). Upper panel: a hit map with the number in each node representing the number of galaxies belonging to that group. In this case 80 means that 80 of the 142 SEDs are classified into the early type group while the 62 of the 142 SEDs are categorized as starburst galaxies. Lower panel: Average SED of the galaxies in each group, early-type on the left and starburst on the right.}
                \label{fig: 1by2V}
            \end{figure}          
            ...
            As mentioned in Sec.~\ref{sec: 1Dt}, in this network size we observed the first separation of the K96 galaxies into 12 different neurons.
           As in Figs.~\ref{fig: 1by2V} and ~\ref{fig: 1by3V}, the upper panel of Fig.~\ref{fig: 1by22V} shows the number of galaxies (out of the 142) belonging to each neuron in the $1\times22$ SOM.
            ...
            If a cluster from the T12 sample fills the same neuron as a K96 template, we can conclude that SEDs of galaxies in the cluster are very similar to those of the template.
            Otherwise, we can conclude that the SEDs of galaxies in the cluster are similar to two of the K96 templates.
            For the latter case, the colours establish which template has greater similarity to the T12 cluster.
            ...
            We therefore conclude that there are no galaxies in the T12 sample with SEDs similar to that of type B or E.
            ...
            In general, 56 galaxies correspond exactly to K96 templates and the other 86 are placed between the initial 12 suggestions.
            As T12 mentioned, part of the reason we could not classify all the galaxies using the K96 model is that the model was constructed from co-adding spectra of 70 galaxies.
...
%PB20160604: this paragraph needs a topic sentence.
        N09 used various models to derive the properties of galaxies.
        ...
        They also studied other relations between properties with no direct correlation in the models in a sample of SINGS galaxies. %%eHMM I will change the previous sentence! It doesn't read well
        ...
        Since in the {\sc CIGALE} code, stellar mass is a free parameter, N09 argued that any stellar-mass-related correlation must be astrophysically meaningful. 
    ...
    \end{figure*}
        ...
        In order to compare our classifications with previous work, we produced similar plots to the ones in both N09 and T12 (Fig.~\ref{fig: props_vs_props}).
        In Fig.~\ref{fig: props_vs_props}, the data points are mean values of the properties of the galaxies in each neuron in Fig.~\ref{fig: 1by22V}, and error bars show the standard deviation of those properties.  % PB20160604: why mean here but median in previous plot?
        ...
        Since in this method we allow galaxies to occupy classes in between the original K96 templates, we have more than 12 colours in Fig.~\ref{fig: props_vs_props}.
        ...
        The upper panels in Fig.~\ref{fig: props_vs_props} show the same relations as N09 and T12, but with tighter correlation than those two, in all three plots.
        ...
        N09 studied a correlation between A$_{\rm {FUV}}$ and other properties of galaxies and showed that the attenuation has no dependence on the specific star formation or age.
        In contrast to N09, we find a correlation between A$_{\rm {FUV}}$ and these two parameters, shown in the lower panels of Fig.~\ref{fig: props_vs_props}.
       ...
       Since our sample has no E-type galaxies, we cannot confidently show the same correlation between A$_{\rm {FUV}}$ and sSFR. 
        ...
        Both \cite{Dale07} and N09 argued that these apparent trends can be a result of the dependence of star formation history on L$_{IR}$/L$_{FUV}$.
...
%    The latter case, can be used to identify outliers. 
    ...
    ``Enough  neurons" is a vague statement, but as mentioned in the Sec.~\ref{sec: method} the size of SOMs is arbitrary and depends on the size of the input data and the kind of information needed from the SOMs. %PB160427: this is a feature of clustering methods in general. Maybe in the methods section where you talk about the size of maps, you could give some examples of what "what kind of information" might mean.
    Since the training dataset (the K96 templates) has 12 members, we assumed a SOM with size of $8\times8$~would be a good start.
    ...
    \begin{figure*}
        ...
        \end{subfigure}
        \caption{$12\times12$~2D SOM results. Upper pabel: SOMs trained using the K96 templates. For training 2D SOMs two different approaches were considered: either only 12 types of galaxies exist (left) or not (right). Lower panel: classifying the galaxies from T12, using the trained networks shown in the upper panels. From the lower right panel, we can see that there are no outliers in the galaxies from T12, and we can use the map in the lower left panel as a final clustering result for the T12 galaxies.}
        \label{fig: 12by12}
    \end{figure*}
    
    The upper left panel in Fig.~\ref{fig: 12by12} shows the SOM results from the first approach. 
    Since we considered that SEDs of all galaxies can be categorized using the K96 templates, the galaxies were placed all over the map.
    Using this network to categorize any set of SEDs forced the SEDs to be either in the same neurons as the K96 templates or in the neurons between them.
    In the map shown in the upper right panel in Fig.~\ref{fig: 12by12}, the K96 templates are in a small region. This provides enough freedom for the SEDs of galaxies to be placed everywhere in the map, even far away from the templates.
    ...
    When categorizing a new set of SEDs, if the new SEDs are similar to the K96 template all of them will be placed in the bottom of the map, but if there are different type of galaxies, they would sit in any other neurons in the map.
    ...
    We used both 2D networks to categorize the T12 galaxies and the results are shown in the lower panels of Fig.~\ref{fig: 12by12}.
    In the lower right map in Fig.~\ref{fig: 12by12}, all galaxies are placed in the bottom part of the map and we can conclude that in the T12 sample there are no outliers or SEDs very different from  the K96 templates.
    The lower left map of Fig.~\ref{fig: 12by12} shows the T12 galaxies categorized based on the network in the upper left of Fig.~\ref{fig: 12by12}. 
