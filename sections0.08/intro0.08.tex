...
%%%%After second referee report
The goal of this work is to compare SOM-based classification of galaxy spectra to two other methods: supervised neural network and (unsupervised) K-means. 
The SOM-based classification involves generating one- and two-dimensional self-organizing maps using the \citetalias{Kinney96} spectral templates. 
These maps are used to classify the spectra of 142 galaxies with $0.5 < z < 1$, the same
sample as analyzed by ~\citet[][hereafter \citetalias{Hossein12}]{Hossein12}.
That work classified spectra using a supervised (multi-layer feed-forward) neural network method and the spectral templates presented by \citetalias{Kinney96}.
The K-means method is used to cluster the \citetalias{Kinney96} and then find distance of each \citetalias{Hossein12} data to centre of clusters to classify them.
...
% haven't figured out where to put this.
