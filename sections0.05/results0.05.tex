...
\label{sec: result}
    ...
    For each grid we created different SOMs with different learning factors, neighbourhood distances, and number of iterations to find the most suitable values for our dataset, which are listed in Section~\ref{sec: create_som}. %%%Sr220616: changed the optimum to the most suitable values ... . moved the initial values to section 3.2
    ...
    \label{sec: 1D}
        ...
        \label{sec: 1Dt}
            ...
            Figs.~\ref{fig: 1by2T} --\ref{fig: 1by22T}~show the results of the training $1\times2$, $1\times3$, and $1\times22$ networks, respectively (The rest of the networks are shown in Appendix~\ref{app: high_Z_1d_soms}.
        ...
        \subsubsection{Classifying the galaxy sample}
         \label{sec: 1Dv}
           ...
           As in Figs.~\ref{fig: 1by2V} and ~\ref{fig: 1by3V}, the upper panel of Fig.~\ref{fig: 1by22V} shows the number of galaxies (out of the 142) belonging to each neuron in the $1\times22$ SOM.
            ...
            %%%%%%%Sr220616: I almost completely changed the following paragraph, but I think it makes more sense, now.
            Other methods for finding the type of galaxies (e.g. $\chi^2$ fitting and supervised neural network) are limited to templates and the outliers are not classified.
            However, using SOM method we categorized galaxies based on their position on trained SOMs, which can have empty neurons.
            The empty neurons in a trained SOM show that there must be another type of template there, that is absent from the K96 template spectra.
            Using the distance of the trained network is the best way to describe the properties of this absent template.
            Therefore, with SOM, we have the freedom to categorize galaxies in intermediate (new) templates.
            %%%%%%%Sr220616: in response to Sophia's comment; In SOM we do not fit the data. we create a network using our template. In the final version of network each neuron has a weight vector. When we give a new data to this weight vector, based on distance of the the weight vector and a new input data, the SOM finds a winner neuron. If this winner neurons were the same for K96 and T12 samples, we would say the input vector in these two case were similar. Since we know type of the galaxies in K96, we could decide about the type of the galaxies in T12. 
        ...
        In order to check whether our clustering in Sec.~\ref{sec: 1Dv}~are correct, in this section we test the relations between median properties of the galaxies in each neuron.  %%%%%%%Sr220616:changed "conclusion" to clustering!
