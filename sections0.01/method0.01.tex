...
\label{sec: method}
 ...
 Results of SOM contains nodes (neurons) that arranged in 1D or 2D arrays \citep{Kohonen98}. 
 ...
 \label{sec: algorithm}
     ...
     In the case of the 1D maps, since each neurons have two immediate neighbours the topology of the map does not have any affect on the final result and we can choose any topology.
     However, in 2D maps, the shape of the neurons specifies the number of the immediate neighbours for each neurons and it is up to user that which shape is more suitable for their data.
     ...
     \begin{enumerate}
        ...
        \item Calculates the euclidean distance for each node, $j$, as  $D_j^2= \sum_{i=0}^{i=n} (V_i - W_i)^2$, and finds a neuron with minimum $D_j$, (``$D_{j_{min}}$"). This neuron is the winner node and is calling Best Matching Unit (BMU). 
...
\label{sec: create_som}
     ...
     The ordering phase continues for requested number of iterations.
     ...
     The number of iterations in this tuning phase most be much more than the number iterations in ordering phase, to allow the tuning happens slowly. %cite kohenen book?!
     ...
     A Hits map, which shows number of the times each neurons become the winner (hits), and a distances map, which shows the same neurons as the hits map and the distance between them.
     In the maps, the purple hexagonal shape areas represent the neurons.
     The distances in a distance map are showing by the grey cycle colours.
...
\subsection{Mock sample}
 ...
 We created  a mock sample of 27 galaxies to show how this method works, illustrate the results of SOMs, and how they can be analyzed.
...
Since the input data are so simple, each entry only could get either 0 or 1 as type of galaxy and 0 or 0.5 as an indicator of  high or low SFR, only four of the neurons were filled.
...
We can concludes that although each galaxy had enough space to occupy any of the neurons in the network, because of the similarity of values they stayed only in four groups.
This network is considered as a train network, and if there is any data set with similar entries, we can use this network to cluster the new data set.
...
However, if the information of the input data are pretty similar to each other, the number of neurons are going to be much higher than number of input samples, to be able to separate the groups from each other. 
